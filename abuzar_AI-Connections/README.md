## Connection Forge MVP Setup

### Add Your Claude API Key

The Claude-powered group generator reads API keys from a local `.env.local` file in the project root.

From this folder:

```bash
cd "/Users/abuzarkhudaverdiyeva/Documents/New project/repo"
cp .env.example .env.local
```

Open `.env.local` and replace the placeholder with your own Claude API key:

```text
ANTHROPIC_API_KEY=your_claude_api_key_here
```

Keep `.env.local` private. Do not submit it, commit it, or share it with anyone else.

Optional model settings are already listed in `.env.example`. The default setup uses Claude Opus for group generation and Claude Sonnet for review when those role-specific variables are enabled.
...
### Generate One Fresh Puzzle From Python
...
That script generates one yellow, green, blue, and purple group, builds a puzzle from them, checks the local database and NYT hash guard, then saves the puzzle if it passes.
