...
OUTPUT_PATH = "yellow_candidate_groups.csv"

RANDOM_SEED = 13
MAX_GROUPS = 5000
GROUPS_PER_BUCKET = 30
MAX_RANDOM_ATTEMPTS_PER_BUCKET = 1200
MAX_SHARED_WORDS_PER_SAME_ANSWER = 2

MIN_HYPERNYM_DEPTH = 6
MAX_HYPERNYM_DEPTH = 8
MIN_BUCKET_SIZE = 4
MAX_BUCKET_SIZE = 110
MAX_HYPONYM_DESCENDANTS = 350

MAX_DISPLAY_WORD_RANK = 80000
MAX_ANSWER_WORD_RANK = 80000
MIN_AVG_PAIRWISE_SIMILARITY = 0.35
MIN_PAIRWISE_SIMILARITY = 0.14
...
def is_good_yellow_hypernym(hypernym):
...
def answer_is_known_and_common(answer, wv):
    for part in answer.split():
        if part not in wv.key_to_index:
            return False

        if wv.key_to_index[part] > MAX_ANSWER_WORD_RANK:
            return False
...
def is_playable_yellow_answer(answer, hypernym, wv):
    if not answer_is_known_and_common(answer, wv):
        return False
...
def is_valid_yellow_display_group(words, answer, wv):
    if len(set(words)) != 4:
        return False, None
...
def hypernym_ancestors_for_synset(synset):
    ...
    for path in synset.hypernym_paths():
        for hypernym in path[:-1]:
            if is_good_yellow_hypernym(hypernym):
                ancestors.add(hypernym)

    return ancestors


def build_yellow_hypernym_buckets(wordlist, wv):
...
def make_candidate_groups(buckets, wv):
    ...
    for hypernym, words in bucket_items:
        ...
        if not is_playable_yellow_answer(answer, hypernym, wv):
            continue
        ...
        for combo in candidate_combos_from_bucket(words):
            combo = list(combo)
            is_valid, coherence_score = is_valid_yellow_display_group(combo, answer, wv)
            ...
            candidates.append({
                "category": "yellow",
                "mechanism": "wordnet_easy_hypernym",
                "word1": combo[0],
                "word2": combo[1],
                "word3": combo[2],
                "word4": combo[3],
...
def main():
    ...
    print("Building yellow WordNet buckets...")
    buckets = build_yellow_hypernym_buckets(wordlist, wv)

    print("Generating yellow candidate groups...")
    groups = make_candidate_groups(buckets, wv)
    save_groups(groups)

    print(f"Saved {len(groups)} yellow groups to {OUTPUT_PATH}")
